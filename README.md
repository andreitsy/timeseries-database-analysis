...
Для того, чтобы поднять проект необходимо установить [docker-compose](https://docs.docker.com/compose/)
```bash
docker compose up --build -d
```

Необходимо после создать логин для `Influx DB`? зарегистрировавшись по адресу [localhost:8086](localhost:8086), получить токен и стандартный бакет `quotes_trades` и организацию.
